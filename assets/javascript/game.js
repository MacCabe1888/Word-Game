...
const game = {
  ...
  play() {
    ...
    game.answerArrayText.textContent = game.answerArray.join("\xa0");
...
game.displayImg.innerHTML = `<img id="display-img" src="${game.imgSrc}">`;
...
document.onkeyup = event => {
  ...
  if (game.guessesLeft === 0 && remainingLetters > 0) {
    ...
    game.lastAnswerImg.classList.remove("hide");
    //sets up new round
    game.play();
  }
  ...
  game.winsText.textContent = "W: " + game.wins;
  game.lossesText.textContent = "L: " + game.losses;
  game.guessesText.innerHTML =
    '<p id="guesses-left-text">Guesses remaining: ' + game.guessesLeft + "</p>"
    + '<p id="your-guesses-text">Your guesses: ' + game.yourGuesses + "</p>";
  game.answerArrayText.textContent = game.answerArray.join("\xa0");
